...
$RootKey = "\Registry\Machine\SOFTWARE\Joakim"
$SubKey = "An Invalid Keyname"
$SpecialValueName = "Secret Value"
$SpecialValueData = "Hidden message"

$CreateKey = _NtCreateKey($RootKey, $SubKey, 8, $SpecialValueName, $SpecialValueData)
If @error Then Exit
_NtFlushKey($CreateKey)
DllCall($hNTDLL, "int", "NtClose", "hwnd", $CreateKey)
DllClose($hNTDLL)
MsgBox(0,"Now open regedit","And verify that you can't open the key: " & $RootKey&"\"&$SubKey)
Exit
